...
{
    private readonly Dictionary<string, List<FactoryMeter>> _cachedMeters = [];
    ...
    public Meter Create(MeterOptions options)
    {
        ...
        lock (_cachedMeters)
        {
            ...
            options.Scope = this;
            FactoryMeter m = new(options.Name, options.Version, options.Tags, scope: this);
            options.Scope = scope;
...
internal sealed class FactoryMeter(string name, string? version, IEnumerable<KeyValuePair<string, object?>>? tags, object? scope) : Meter(name, version, tags, scope)
{
