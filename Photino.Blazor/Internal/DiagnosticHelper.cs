...
internal static class DiagnosticsHelper
{
    ...
    internal static bool CompareTags(List<KeyValuePair<string, object?>>? sortedTags, IEnumerable<KeyValuePair<string, object?>>? tags2)
    {
        ...
        BitMapper bitMapper = new(size <= 100 ? stackalloc ulong[size] : new ulong[size]);
            ...
            if (tagsCol is IList<KeyValuePair<string, object?>> secondList)
            {
                for (int i = 0; i < count; i++)
                {
                    ...
                    for (int j = 0; j < count; j++)
                    {
                        ...
                        if (compareResult == 0 && Equals(pair.Value, pair1.Value))
                        {
                            bitMapper.SetBit(j);
                            break;
                        }
...
internal readonly ref struct BitMapper
{
    ...
    public readonly int MaxIndex => _maxIndex;
    ...
    public readonly bool SetBit(int index)
    {
